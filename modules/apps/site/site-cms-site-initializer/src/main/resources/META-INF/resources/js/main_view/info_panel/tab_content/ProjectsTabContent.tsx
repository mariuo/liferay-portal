/**
 * SPDX-FileCopyrightText: (c) 2026 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

import React, {useContext} from 'react';

import CMPProjectsMultiSelect from '../../../common/components/CMPProjectsMultiSelect';
import {
	AssetTypeInfoPanelContext,
	IAssetTypeInfoPanelContext,
} from '../context';

const ProjectsTabContent = () => {
	const {
		asset,
		assetLibrary,
		cmpProjectAssetRelationshipObjectDefinitionId,
		cmpProjectObjectDefinitionId,
		selectedAssets,
	}: IAssetTypeInfoPanelContext = useContext(AssetTypeInfoPanelContext);

	return (
		<CMPProjectsMultiSelect
			cmpProjectAssetRelationshipObjectDefinitionId={
				cmpProjectAssetRelationshipObjectDefinitionId ?? null
			}
			cmpProjectObjectDefinitionId={cmpProjectObjectDefinitionId ?? null}
			entryClassName={selectedAssets[0]?.entryClassName ?? ''}
			entryExternalReferenceCode={asset?.externalReferenceCode ?? ''}
			entryGroupExternalReferenceCode={
				assetLibrary?.externalReferenceCode ?? ''
			}
		/>
	);
};

export default ProjectsTabContent;
